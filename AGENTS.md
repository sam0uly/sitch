...
- `internal/system` is the platform-specific collection of `Info`. Two build tags split reality:
  ...
  - `metrics_darwin.go` and `metrics_windows.go` provide real `memory()`/`diskUsage()`/`osAge()` for macOS (`vm_stat` output + `hw.memsize` sysctl; used = total - free - inactive) and Windows (`kernel32!GlobalMemoryStatusEx` via lazy proc, `GetDiskFreeSpaceEx`, `%SystemDrive%\Windows` creation time). All report MiB and "N days", matching the Linux formats.
  - `platform_other.go` (darwin+windows `collectPlatform`), `metrics_other.go` (`!linux && !darwin && !windows` stubs), and `gpu_other.go` are no-op fallbacks. Most fields return `"unknown"`, and there is no `osRelease`, no `packageCounts`, no `gpuModels`.
  - `memory()` takes no arguments and reads its source per platform; on Linux the parseable unit is `parseMeminfo(raw)`.
